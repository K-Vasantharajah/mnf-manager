"""
Ratings routes — exposes ML-derived player ratings calculated from
the ridge regression impact model.
"""
...
def get_ratings():
    ...
    try:
        ...
        return jsonify({"status": "success", "count": len(ratings), "ratings": ratings})
...
def get_player_rating(player_id):
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "rating": player.to_dict(orient="records")[0]}
        )
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
