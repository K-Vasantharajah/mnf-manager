...
def calculate_derived_ratings(force_refresh: bool = False):
    ...
    rateable["attack_rating"] = (
        scaler.transform(rateable[["attack_impact"]].values).round(0).astype(int)
    )

    rateable["defence_rating"] = (
        scaler.transform(rateable[["defence_impact"]].values).round(0).astype(int)
    )
